...
def get_overlap2(bbox1, bbox2):
	'''
	Calculates the degree of overlap of two bounding boxes.
	This can be any value from 0 to 1 where 0 means no overlap and 1 means complete overlap.
	This function has a different normalization than get_overlap
	The degree of overlap is the ratio of the area of overlap and the average area of the two boxes.
	'''
	overlap_area=get_overlap_area(bbox1,bbox2)

	bbox1_area = get_area(bbox1)
	bbox2_area = get_area(bbox2)
	# now I'm using average. In the future I may want to change it to somenting else like (big_area + 2*small_area)/3 or (big_area*small_area)**0.5
	# this is why I'm calling it dividing_area 
	dividing_area = (bbox1_area + bbox2_area)/2

	#epsilon = 1e-5 # small value to prevent division by zero
	overlap = overlap_area / dividing_area
	return overlap
